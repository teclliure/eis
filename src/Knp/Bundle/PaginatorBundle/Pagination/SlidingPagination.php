...
class SlidingPagination extends AbstractPagination
{
    ...
    public function getSort($key, array $params = array())
    {
        $params = array_merge($this->params, $params);

        if (isset($params[$this->getPaginatorOption('sortFieldParameterName')]) && $params[$this->getPaginatorOption('sortFieldParameterName')] === $key)  {
           return $params[$this->getPaginatorOption('sortDirectionParameterName')];
        }
        else {
          return false;
        }
